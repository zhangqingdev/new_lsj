package com.oushangfeng.lsj.utils;

public class MainConstants {
	/** debug*/
	public static final boolean debug = false;
	/** 设备ID **/
	static final String DEVICE_ID = "imei";

	static final String IMSI_ID = "imsi";

	static final String DEVICE_FIRM = "mobileFirm";
	/** 设备名称 **/
	static final String DEVICE_NAME = "device_name";

	/** 当前应用的packageName **/
	static final String CURRENT_PACKAGE_NAME = "app_package_name";

	/** 当前应用的versionCode **/
	static final String CURRENT_VERSION_CODE = "appVersionCode";

	/** 设备所用平台的类型 **/
	static final String OS_TYPE = "osType";

	/** 设备上平台的版本 **/
	static final String OS_VERSION = "osCode";

	/** 设备所处的国家编码 **/
	static final String COUNTRY_CODE = "country_code";

	/** 设备所处地区的语言 **/
	static final String LANGUAGE = "language";

	/** 程序的版本 **/
	static final String APP_VERSION = "appVersionName";

	/** 持有设备的用户编号/用户账户 **/
	static final String USER_ID = "snuid";
	/** 设备屏幕的密度 **/
	static final String SCREEN_DENSITY = "screen_density";

	/** 设备占用尺寸 **/
	static final String SCREEN_LAYOUT_SIZE = "screen_layout_size";

	/** 屏幕横向分辨率 **/
	static final String SCREEN_WIDTH = "screen_width";

	/** 屏幕纵向分辨率 **/
	static final String SCREEN_HEIGHT = "screen_height";

	/** 联网方式 **/
	static final String NET_WORK_TYPE = "net";

	static final String EMULATOR_ID = "emulator0000000";


	public final static String ANDROID_ID="android_id";
	public final static String SERIAL_NUMBER ="serialnumber";
	//渠道号
	public final static String CHANNEL="channelId";
	//邀请码
	public final static String UUID="investId";

	public static final String QQ_APPID="1106063084";
	public static final String WB_APPID="";
	public static final String WB_REDIRECT_URL="";
	public static final String WB_SCOPE="";
	public static final String WX_APPID="";
	public static final String ACTION_WX_SHARE="";
	public static final String ACTION_WX_AUTH="";
	public static final String SCHEME = "lsj";
}
